#!/bin/sh
# caged: map the shared ATLASSION_* env trio the operator injects
# (ATLASSION_HOST / ATLASSION_EMAIL / ATLASSION_API_TOKEN) onto the env vars
...
# it was started. CLI-specific vars win when already set; ATLASSION_URL is
...
_atlassian_host="${ATLASSION_URL:-${ATLASSION_HOST:-}}"
...
CFL_EMAIL="${CFL_EMAIL:-${ATLASSION_EMAIL:-}}"
JIRA_LOGIN="${JIRA_LOGIN:-${ATLASSION_EMAIL:-}}"
CFL_API_TOKEN="${CFL_API_TOKEN:-${ATLASSION_API_TOKEN:-}}"
JIRA_API_TOKEN="${JIRA_API_TOKEN:-${ATLASSION_API_TOKEN:-}}"
